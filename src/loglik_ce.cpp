...
//' @noRd
 ...
 Rcpp::List ptmvt_ce(Rcpp::NumericVector gamma, Rcpp::List model) {
   ...
   for (int i = 1; i < n; ++i) {
     ...
     // st^2 = (nu + d_{<t})/(nu + t - 1) * s_t^2
     double st = std::sqrt(v[i] * sigma2*(nu + d)/(nu+i));
     double num = t_utils::t_pdf((r[i]-mt[i])/st, nu+i) / st;
     double den = t_utils::t_pdf(r[i], nu);
     llk[i] = log(num) - log(den) + std::log(a[i]);
     d +=  (r[i] - mt[i])*(r[i] - mt[i])/(st*st);
   }
