...
arma_order <- c(1, 1)
tau <- c(phi,theta)
sim_data <- sim_poisson(mu, tau, arma_order, nsim = n, seed = 22, family ="gaussian")
y <- sim_data$y
x <- matrix(1, nrow = n)

system.time({
  fit_CE_test <- gctsc(y~1,
                  marginal = poisson.marg(lambda.lower = c(0)),
                  cormat = arma.cormat(p = 1, q = 1),family="gaussian",
                  method = "GHK_test", c=0.5,
                  QMC = TRUE)
})
...
system.time({
  fit_CE <- gctsc(y~1,
                        marginal = poisson.marg(lambda.lower = c(0)),
                        cormat = arma.cormat(p = 2, q = 1),family="gaussian",
                        method = "CE",
                        QMC = TRUE)
})


predict(fit_GHK_test)
...
fit_fml <- gctsc(Y ~ x2 + x3 + x4, data = data_train,
                 marginal = poisson.marg(link = "log"),
                 cormat = arma.cormat(p = 1, q = 0),
                 method = "GHK",
                 options = gctsc.opts(seed = 1))
