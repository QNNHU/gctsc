loglik_vmet <- function(ab, tau, M , od, QMC,
            ret_llk, df, family){
  result <-  switch (family,
            "gaussian" = pmvn_vmet(lower =ab[,1],upper = ab[,2], tau, M, od, 
                                   QMC = QMC, ret_llk = ret_llk),
            "t" = pmvt_vmet(lower =ab[,1],upper = ab[,2], tau, M, od, 
...
#'
#' @export
pmvn_vmet <- function(lower,upper, tau, M = 1000, od, QMC = TRUE, ret_llk = TRUE) {
  ab <- cbind(lower, upper)
  ...
  result <- tryCatch({
    VeccTMVN::pmvn(lower = lower, upper = upper, sigma = covM,
...
#'
#' @export
pmvt_vmet <- function(lower,upper, tau, M = 1000, od, QMC = TRUE, ret_llk = TRUE, df= NULL) {
  ab <- cbind(lower, upper)
  if (length(tau) != sum(od)) {
    stop("Length of 'tau' must equal sum of AR and MA orders: length(tau) = ",
         length(tau), ", expected = ", sum(od), ".")
